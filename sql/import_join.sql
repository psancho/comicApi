select
    ...
    I.`J'ai`,
    case
        when Z.titre is null then 0 else 2
    end is_last,
    case
        when I.`Série` in ('BRETECHER', 'ENKI BILAL', 'LOUP', 'MANARA', 'MORDILLO', 'REISER', 'SERRE', 'WOLINSKI') then 4 else 0
    end is_author_series,
    case
        when I.`Série`= 'AUTEURS' then 8 else 0
    end is_biopic,
    */
    I.`J'ai`
    + case
